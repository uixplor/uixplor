        ...
        jsonLd={[
          {
            ...
            description:
              "In-depth CSS guides and UI design tutorials for modern web developers.",
                    ...
                    <div className="mb-8">
                      ...
                      >
                        <div className="rounded-2xl overflow-hidden border border-white/6 hover:border-white/14 transition-all duration-300 hover:shadow-[0_4px_20px_rgba(255,255,255,0.06)] flex flex-col sm:flex-row">
                          <div
                            className="sm:w-64 lg:w-80 shrink-0 h-40 sm:h-auto"
                          ...
                          <div className="flex-1 p-5 sm:p-6 bg-white/0.025">
                            <div className="flex flex-wrap gap-1.5 mb-3">
                              {featured.tags.slice(0, 3).map((tag) => (
                                <span
                                  key={tag}
                                  className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border uppercase tracking-wider ${tagClass(tag)}`}
                                >
                                  {tag}
                                </span>
                              ))}
                              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#B8FB3C]/10 text-[#B8FB3C] border border-[#B8FB3C]/20 uppercase tracking-wider">
                                Featured
                              </span>
                            </div>
                            <h2 className="text-lg sm:text-xl font-bold text-white mb-2 group-hover:text-[#B8FB3C] transition-colors duration-300 leading-snug">
                              {featured.title}
                            </h2>
                            ...
                            <div className="flex items-center gap-3 text-xs text-white/30">
                              ...
                              <span className="ml-auto text-[#B8FB3C] font-semibold group-hover:translate-x-1 transition-transform duration-300">
                                Read →
                              </span>
                            </div>
                          </div>
                        ...
                        >
                          ...
                          >
                            <div className="h-full rounded-2xl overflow-hidden border border-white/6 hover:border-white/14 transition-all duration-300 hover:shadow-[0_4px_16px_rgba(255,255,255,0.05)] flex flex-col">
                              ...
                              >
                                <div className="h-full flex items-center justify-center">
                                  <span className="text-white/15 font-black text-5xl select-none">
                                    {post.tags[0]?.[0] ?? "C"}
                                  </span>
                                </div>
                              </div>
                              <div className="flex-1 p-4 flex flex-col">
                                <div className="flex flex-wrap gap-1 mb-2">
                                  {post.tags.slice(0, 2).map((tag) => (
                                    <span
                                      key={tag}
                                      className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full border uppercase tracking-wider ${tagClass(tag)}`}
                                    >
                                      {tag}
                                    </span>
                                  ))}
                                </div>
                                <h2 className="text-sm font-semibold text-white mb-1.5 leading-snug group-hover:text-[#B8FB3C] transition-colors duration-300 line-clamp-2">
                                  {post.title}
                                </h2>
